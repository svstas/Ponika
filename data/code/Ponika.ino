...
const char * const  psensors[] PROGMEM = {sens0,sens1,sens2,sens3,sens4,sens5,sens6,sens7,sens8,sens9,sens10,sens11,sens12};
...
Adafruit_NeoPixel pixels = Adafruit_NeoPixel(8, 9, NEO_GRB + NEO_KHZ800);
...
std::vector<String> dlslist;
  ...
  void addSensorI(char * sname, uint8_t xdelay, std::function<int16_t()> callback, bool isint,uint8_t id) {
    Sensor one;
    one.id = id;
    one.isint = false;
    one.sdelay = xdelay;
...
//     digitalWrite(2,lamp); if (lamp==LOW) lamp=HIGH; else lamp=LOW;
...
//             sendEvent(">>"+String(i)+" : "+String(LSensors[i].value),"sms");
...
bool readPcf(uint8_t pin) {
  return pcf1.read(conf.pcfr_swap[pin]);
}
...
void sort() {std::sort(vc.begin(), vc.end(), [] (Lcron const& a, Lcron const& b) { return a.tstart < b.tstart; });}
...
void loadRel() {
  ostate = statex;
  for(uint8_t i=0;i<8;i++) {relay[i] =  pcf1.read(conf.pcfr_swap[i]);orelay[i] = relay[i];}
}
...
void delC() {ptr--;vc.erase(vc.begin()); vector<Lcron>(vc).swap(vc);}
...
for(int i=0;i<8;i++) {
  ...
  if (irelay[i]==0) {
    if (orelay[i]!=relay[i]) {if (relay[i]==rstat[i]) irelay[i]=1; else irelay[i]=2; orelay[i]=relay[i];} 
  }
...
//    if ((sec==1 || sec==10 || sec==20 || sec==30 || sec==40 || sec==50 ) && cron.maintance) savesens();

    if ((sec==1) && cron.maintance) savesens(); 
    evtSync(false);
