...
void reset_wifi_set() {
//SPIFFS.rename("/admin/ini/config.ini","/admin/ini/config.ini.old");
...
String out = "";
  ...
  while(f.available()) {
    ...
    par.trim();
    if (par=="wifiSSID") xs = "wifiSSID = "+String(conf.ssid);
    if (par=="wifiPASS") xs = "wifiPASS = "+String(conf.password);
    out+=xs+"\n";
  }
f.close();

f = SPIFFS.open("/admin/ini/config.ini", "w");
f.print(out);
f.close();
//Serial.println(out);

//SPIFFS.remove("/admin/ini/config.ini.old");
delay(1000);
Serial.println("reset passwd");
}
...
void loadConfig(bool sens) {
  ...
  while(f.available()) {
    ...
    if (par=="wifiSSID") strcpy(conf.ssid,val.c_str());
    if (par=="wifiPASS") strcpy(conf.password,val.c_str());    
...
//    if (par=="autoreboot") autoreboot=val.toInt();
    ...
    if (par=="pixelPin") conf.pixelPin=val.toInt();
    if (par=="colors") {
...
//      if (xtm-savedTime>3) {
//        ESP.reset();
//        sendEvent(String(savedTime)+":"+String(xtm),"sms");
//        sendEvent("!!!","sms");
//      }
 //     savedTime = xtm;
      
    datas=String(xtm)+"|100:"+ESP.getFreeHeap()+"|";
    f.print(datas);
...
//vall = mysensors->LSensors[i].value;

    String xv = String(vall);
    if (xv!="nan") {
        datas = datas+String(mysensors->LSensors[i].id)+":";
        f.print(mysensors->LSensors[i].id);
        f.print(":");
        f.print(xv);
        datas += xv;
        if (i<(mysensors->pt-1)) {f.print("|");datas+="|";}
    }
}
    if (xrr) {f.print(xrr);datas+=xrr;}
